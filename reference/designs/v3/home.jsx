...
function StatsStrip({ t }) {
  const stats = [
    { n: "25", l: t("Neighborhood clinics", "Clínicas") },
    { n: "50+", l: t("Pediatric providers", "Proveedores") },
    { n: "30", l: t("Years caring for LA", "Años de servicio") },
    { n: "4", l: t("Languages spoken", "Idiomas") }];

  return (
    <section className="stats-strip">
      <div className="wrap">
        {stats.map((s, i) =>
        <div key={i} className="stat-item" style={{ transitionDelay: (i * 70) + "ms" }}>
...
function HomePage({ tw, t, nav, onBook, onLocations }) {
  React.useEffect(() => {
    const root = document.querySelector(".kt-root") || document.body;
    const sel = ".care-card, .partner-logo-card, .res-card, .story-card, .ecp, .stat-item, .sec-head, .insurance, .ef-card";
    const els = Array.from(document.querySelectorAll(sel));
    els.forEach((e) => e.classList.add("v3reveal"));
    const reveal = () => root.classList.add("v3-anim");
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduce || !("IntersectionObserver" in window)) {
      // no animation: don't add .v3-anim, so .v3reveal stays fully visible
      return;
    }
    reveal();
    const showIfNear = () => {
      ...
      els.forEach((e) => {
        if (e.classList.contains("v3in")) return;
        const r = e.getBoundingClientRect();
        if (r.top < vh * 0.92) e.classList.add("v3in");
      });
    };
    const io = new IntersectionObserver((entries) => {
      entries.forEach((en) => { if (en.isIntersecting) { en.target.classList.add("v3in"); io.unobserve(en.target); } });
    }, { threshold: 0.1, rootMargin: "0px 0px -8% 0px" });
    els.forEach((e) => io.observe(e));
    showIfNear();
    window.addEventListener("scroll", showIfNear, { passive: true });
    // safety net: if anything is still hidden after 4s, drop the gate so all content shows
    const safety = setTimeout(() => root.classList.remove("v3-anim"), 4000);
    // safety net for hero entrance: guarantee hero content is visible even if its
    // CSS animation is throttled (e.g. backgrounded tab) and never advances
    const heroSafety = setTimeout(() => {
      document.querySelectorAll(".hero.feature .hero-feature-content > *").forEach((e) => {
        e.style.opacity = "1";
        e.style.transform = "none";
        e.style.animation = "none";
      });
    }, 2000);
    return () => { io.disconnect(); window.removeEventListener("scroll", showIfNear); clearTimeout(safety); clearTimeout(heroSafety); };
  }, []);
  return (
    <>
...
Object.assign(window, { LocationsFinder, Hero, CareSection, Partners, Resources, Insurance, Stories, CtaBand, CareersPromo, EventBanner, StatsStrip, HospitalNewborn, HomePage });