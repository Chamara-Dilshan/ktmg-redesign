          ...
          <FadeIn className="lg:col-span-1 lg:row-span-2">
            ...
            >
              ...
              >
                ...
                <div>
                  <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-white/[0.1] text-white ring-1 ring-white/10">
                  ...
                  {tFeatured.hours && (
                    <p className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-coral/20 px-3 py-1 text-xs font-semibold text-coral">
                      <ClockDot className="h-3.5 w-3.5" />
                      {tFeatured.hours}
                    </p>
                  )}
            ...
            return (
              <FadeIn key={service.slug} delay={i * 0.06}>
                ...
                >
                  ...
                  >
                    {/* Accent bar — full-width hairline, colored by category */}
                    <div className={`h-[3px] w-full shrink-0 ${isUrgent ? 'bg-coral' : 'bg-teal-mid'}`} />
                    ...
                    <div className="flex flex-1 flex-col p-6">
                      ...
                      {ts.hours && (
                        <p className={`mb-2.5 inline-flex w-fit items-center gap-1.5 text-[11px] font-semibold ${isUrgent ? 'text-coral-ink' : 'text-teal-mid'}`}>
                          <ClockDot className="h-3 w-3" />
                          {ts.hours}
                        </p>
                      )}
                      ...
                      <motion.span
                        ...
                        className={`flex items-center gap-1.5 text-xs font-semibold transition-colors group-hover:text-coral-ink
                          ${isUrgent ? 'text-coral-ink' : 'text-teal-mid'}`}
                      >
                        {t('servicesGrid.learnMore')} →
                      </motion.span>
                    </div>
          ...
          <FadeIn delay={0.24} className="lg:col-span-3">
            ...
            >
              <Link
                ...
                className="group flex flex-col gap-5 overflow-hidden rounded-2xl border border-teal-mid/15 bg-gradient-to-r from-teal-tint/70 to-white p-6 sm:flex-row sm:items-center"
              >
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-teal-dark text-white ring-1 ring-teal-dark/10 transition-transform duration-300 group-hover:scale-105">
                  <ServiceIcon slug={wideCard.slug} className="h-7 w-7" />
                </div>

                <div className="min-w-0 shrink-0 sm:w-56">
                  <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.16em] text-teal-mid">{t('servicesGrid.alsoAvailable')}</p>
                ...
                <div className="hidden h-8 w-px shrink-0 bg-teal-mid/20 sm:block" />

                <div className="flex flex-wrap gap-2">
                  {[1, 2, 3, 4].map(n => (
                    <span key={n} className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-teal-mid ring-1 ring-teal-mid/20">
                      {t(`serviceData.specializedCare.tag${n}`)}
                    </span>
                  ))}
                </div>
                ...
                <motion.span
                  whileHover={{ x: 3 }}
                  transition={{ type: 'spring', stiffness: 400 }}
                  className="hidden shrink-0 items-center gap-1.5 text-xs font-semibold text-teal-mid transition-colors group-hover:text-coral-ink sm:ml-auto sm:flex"
                >
                  {t('servicesGrid.learnMore')} →
                </motion.span>
              </Link>
